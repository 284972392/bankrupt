<template>
  	<div class="mediaRegister">
  		<el-form :model="form" ref="form" class="formBox">
  		<div class="content">
  			<div class="left">
			  		...
			  		<div class="p2">账户类型</div>
						<el-radio disabled  v-model="radio" label="1">直客</el-radio>
			  		
  			</div>
  			<div class="right">
			  		<div class="p3">联系人信息</div>
			        ...
			        <el-form-item prop="username" :rules="{validator:validator_adr,trigger:'blur'}">
			          <el-input v-model="form.username" @keyup.enter.native="submit('form')" placeholder="请输入联系人的真实姓名">
			         
			          </el-input>
			        </el-form-item>
			        <el-form-item prop="mobile" :rules="{validator:validator_ZZnumber,trigger:'blur'}">
			          <el-input v-model="form.mobile" @keyup.enter.native="submit('form')" placeholder="请输入真实有效的手机号码">
  			...
  			<div class="operation">
                <input type="checkbox" id="mycheck3" v-model="ischeck">
                <label for="mycheck3"></label>
                <label for="mycheck3"><span>我已阅读并同意</span></label>
...
export default {
	props:["message"],//接收父组件参数，用于区分注册广告主还是媒体
  name: 'mediaRegister',
  components: {
  },
  data() {
     ...
     validator_adr(rule, value, callback){
      ...
      if (value === '' || value === undefined || value === null) {
        ...
        callback()
  		...
  		submit(formName){
  			console.log(this.whichRegister)
