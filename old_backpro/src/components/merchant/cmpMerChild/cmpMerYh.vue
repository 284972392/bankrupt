	...
	export default{
		...
		methods:{
			...
			handlePageSize(value){
					this.get(this.allUrl.findDiscountPageBytenantId,{page:value,rows:9,tenantId:this.tenantId}).then((res)=>{
		...
		mounted(){
			...
			this.get(this.allUrl.findDiscountPageBytenantId,{page:1,rows:9,tenantId:this.tenantId}).then((res)=>{
				console.log(res)
                this.totalNum = res.data.total
        		this.data1 = res.data.rows;
        		 res.data.rows.forEach((i,index)=>{        
