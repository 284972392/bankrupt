	...
	export default{
		...
		methods:{
			checkRecharge(params){
				this.$Message.warning("在渠道管理里面查看")//由于当初任务是 我与苏世博分开写的， 只读页面的入口不一样，  还要分任务种类。。 暂时不做
			},
			handlePageSize(value){
                this.get(this.allUrl.findTaskPageBytenantId,{page:value,rows:9,tenantId:this.tenantId}).then((res)=>{
				console.log(res)
			...
			this.get(this.allUrl.findTaskPageBytenantId,{page:1,rows:9,tenantId:this.tenantId}).then((res)=>{
				console.log(res)
                this.totalNum = res.data.total
        		this.data1 = res.data.rows;
