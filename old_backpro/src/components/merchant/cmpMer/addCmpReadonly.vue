	...
	<div class="addCmpReadonly">
		...
		<div class="cmpMerBase">
			<div class="merBack">
				...
				<div>手机号码<span>{{base.mobiPhone}}</span></div>
				<div>QQ<span>{{base.qq}}</span></div>
				<div class="zhiZhao"><span>工商营业执照</span><img :src=base.idCardPic alt="" /></div>
				<div class="merLogo"><span>公司logo</span> <img :src=base.headPic alt="" /></div>			
			</div>
			<div class="merPro">
...
<script>
	export default{
		...
		methods:{
			toAddCmp(){
            	this.$router.push({path:'/addCmpMer',query:{id:this.id}})				
			},
			firstChange(value){
        		if(value == 1){
        			this.product = true;
