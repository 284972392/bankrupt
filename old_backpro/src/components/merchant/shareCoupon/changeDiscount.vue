             ...
             <Form :model="formItem" :label-width="130" class="form">
                  ...
                  <Row>
                    <Col span="7" offset="1">
                        ...
                        <FormItem label="卡券详情样式    *">
                            <lazy :time='200'>
                             <addpic @update="detailStyle" @remove="removeDe" :size='1' :defaultPic='formItem.cardDetailPic'></addpic>
                             </lazy>
                        </FormItem>        
                        <FormItem label="领取人数     *">
                    ...
                    <Col span="7" offset="2">
                        ...
                        <FormItem label="卡券列表样式    *">
                            <lazy :time='200'>
                              <addpic @update="listStyle" @remove="removelist" :size='1' :defaultPic='formItem.cardListPic'></addpic>
                             </lazy>
                        </FormItem>   
                        <FormItem label="优惠简介    *">
                ...
                <!-- 添加领取说明 -->
                 <div class="addInfo" v-for="(data,index) in this.formItem.receiveVoList">
                    ...
                    <Col span="7" offset="2">
                        <FormItem label="攻略图片">
                              <addpic @update="strategy(index)" @remove="removeStrategy(index)" :size='3' :defaultPic='data.strategyPic'></addpic>
                        </FormItem>   
                    </Col>
                  </Row>
                 </div>
                        ...
                        <Col span="7">
                            <FormItem label="领取方式    *">
                                <Select v-model="formItem.receiveMode" @on-change="changeGet()">
                                    <Option v-for="item in getway" :value="item.value" :key="item.value">{{ item.label }}</Option>
                                </Select>
                            </FormItem>  
                         </Col>
                         <Col span='7'>
                              <FormItem label="页面跳转方式    *" v-if="formItem.receiveMode=='单链接可跳转'||formItem.receiveMode=='多个链接'">
                                <Select v-model="formItem.pageTurn">
                                    <Option v-for="item in jumpway" :value="item.value" :key="item.value">{{ item.label }}</Option>
                                </Select>
                             </FormItem>   
                         </Col>
                        </Row> 
                ...
                <div style="margin-left:40px;margin-top:20px;"><Button size='large' type="primary" @click="addLink">添加领取链接</Button></div>
                 <div class="addInfo" v-for="(data,index) in this.formItem.linkVoList ">
                    ...
                    <Col span="7" offset="2">
                        ...
                        <FormItem label="logo    *">
                              <addpic @update="logo(index)" @remove="removeLogo(index)" :size='3' :defaultPic='data.logo'></addpic>
                        </FormItem>   
                    </Col>
                  </Row>
                 </div>
    ...
    export default {
        ...
        methods: {
            ...
            soldOutTime(data){
                ...
                console.log(this.formItem.theShelvesTime)
                
            },
            putawayTime(data){
                this.formItem.shelvesTime = data;
                console.log(this.formItem.shelvesTime)
            },
            putinTime(data){
                this.formItem.putInTime = data[0]+","+data[1];
            },
            // 添加领取说明
            addExp(){
                let a = {
            ...
            strategy(index){
                 // 如果是字符串,将其转换为数组
                if(typeof(this.formItem.receiveVoList[index].strategyPic)=='string'){
                    this.formItem.receiveVoList[index].strategyPic = this.formItem.receiveVoList[index].strategyPic.split(',')
                }
            ...
            // 获取logo图片
            logo(index){
                if(typeof(this.formItem.linkVoList[index].logo)=='string'){
                    this.formItem.linkVoList[index].logo = this.formItem.linkVoList[index].logo.split(',')
                }
                   ...
                   return;
            		...
            		if (this.flag == true&& this.flag2 ==true){
                      this.formItem.putInTime = this.week.join(',')+'*'+this.hour.join('-')
                        ...
                        this.$http({
                                method:'POST',
                                url: this.allUrl.updateDiscount,
                                headers: {
        ...
        beforeMount () {
             let query = this.$route.query
              ...
              this.get(this.allUrl.findDiscount,{discountId:query.id}).then((res)=>{
                  console.log(res)
