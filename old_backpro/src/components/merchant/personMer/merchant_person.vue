<template>
    <div class="home">
        ...
        <ButtonGroup size="large">
             <Button type="primary" ghost @click="addPersonMer">添加</Button>
        </ButtonGroup>
        </div>
...
import foot from '@/components/footer.vue'
    ...
    export default {
        ...
        methods:{
        	...
        	toAddPerson(){
        		this.$router.push({path:'/addPersonMer',query:{id:this.merId}})
        	},
        	firstChange(value){
        		if(value == 1){
        			this.product = true;
        	...
        	//获取列表
        	AllTask(){
        		this.get(this.allUrl.findTenantPage,{page:1,rows:9,type:1}).then((res)=>{
        	...
        	secondChange(value){
        	  ...
        	  //修改个人商户
            changeCmpMer(params){
            	this.$router.push({path:'/addPersonMer',query:{id:params.row.id}})
            },
             handleSelectAll () {
            ...
            Charge(){
             ...
             handlePageSize(value){
                this.get(this.allUrl.findTenantPage,{page:value,rows:9,type:1}).then((res)=>{
                    this.totalNum = res.data.total
                    this.data1 = res.data.rows
                    res.data.rows.forEach((i,index)=>{        
                    i.createTime = this.changeTime(i.createTime)
            ...
            //查询商户
            findRoleName(){
            	this.get(this.allUrl.findTenantPage,{page:1,rows:9,type:1,name:this.merName}).then((res)=>{
                this.totalNum = res.data.total
        		this.data1 = res.data.rows;
        		 res.data.rows.forEach((i,index)=>{        
                  i.createTime = this.changeTime(i.createTime)
            ...
            //查看个人商户详情
            checkPersonMer(params){
            	localStorage.setItem("merSherchId",params.row.id)
            	this.$router.push({path:'/personMerSherch',query:{id:params.row.id}})
            },
            //添加个人商户
            addPersonMer(){
            	this.$router.push('/addPersonMer')
            },
            ok(){
            	//办卡赚跳转
