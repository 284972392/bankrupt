<template>
	<!--商户名称 编号需要从上级页面传过来-->
	<div class="cmpMerMission">
		<div class="billList">任务信息</div>
		<div class="billBox" v-for="data in billList">
			<table border="" cellspacing="0" cellpadding="0">
				<tr class="thd"><td>序号</td><td>任务编号</td><td>任务名称</td><td>任务总价（元）</td><td>发布日期</td><td>状态</td><td>操作</td></tr>
				<tr><td>{{data.id}}</td><td>{{data.number}}</td><td>{{data.name}}</td><td>{{data.sum.toFixed(2)}}</td><td>{{data.billTime}}</td><td>{{data.billTime}}</td><td class="look" @click="look(data)">查看</td></tr>
			</table>
		</div>
