...
<script>
    ...
    export default {
        ...
        methods: {
            save(){
                if(this.addr.length==0||this.formItem.name==''||this.formItem.logo==''||this.formItem.addressDetail==''){
					...
					this.formItem.address = this.formItem.address +"/"+this.formItem.addressDetail
                    if(!this.isChange){
                    	console.log(this.formItem)
                         this.$http({
                        method:'POST',
                        url:this.allUrl.addEducationOrganization,
                        headers: {
                            ...
                            if(res.data.errordesc==0){
                                 this.$Message.success('保存成功！');
                                 this.$router.push('/haha_study/institutions_list')
                            }
                        })
                    }else{
                        this.$http({
                        method:'POST',
                        url:this.allUrl.updateEducationOrganization,
                        headers: {
                                "content-type": "application/json"
                            },
                            responseType:'text/plain',
        ...
        beforeMount () {
            ...
            if(query.id){
                this.isChange = true
                    this.formItem.name = query.name
                    this.formItem.logo = query.logo
                    let arr = query.address.split('/');
                    this.addr = arr.slice(0,arr.length-1)
                    let addressDetail = arr.pop();
                    this.formItem.addressDetail = addressDetail
