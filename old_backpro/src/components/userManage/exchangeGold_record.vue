<template>
    <div class="exchangeGole">
        <div class="title">金币兑换记录</div>
        <div class="search">
            搜索
       ...
       <div class="state_con">
            ...
            <div class="btn_state" :class="{active:shows==2}" @click="success()">兑换成功({{goldNum.success}})</div>
            <div class="btn_state" :class="{active:shows==3}" @click="fail()">兑换失败({{goldNum.fail}})</div>
     ...
     <Modal v-model="showForm" footer-hide  scrollable  :styles="{top: '200px'}">
         ...
         <Row>
             <Col span="12">
                ...
                <p style="height:40px;font-size:15px;">兑换数量（个）&nbsp;&nbsp;&nbsp; {{goldContent.coinNumber}}个</p>
                <p style="height:40px;font-size:15px;">兑换时间&nbsp;&nbsp;&nbsp; {{goldContent.createTime}}</p>

             </Col>
             <Col span="12" offset="0">
                  <p style="height:40px;font-size:15px;">用户编号&nbsp;&nbsp;&nbsp; {{goldContent.userId}}</p>    
                  <p style="height:40px;font-size:15px;">兑换编号&nbsp;&nbsp;&nbsp; {{goldContent.id}}</p>
                  <p style="height:40px;font-size:15px;">兑换状态&nbsp;&nbsp;&nbsp; {{goldContent.coinStatus}}</p>
    ...
    export default {
        ...
        methods:{
            ...
            allRecord(){
                ...
                this.shows = 1
                this.value = ''
                 this.get(this.allUrl.findExchangeAppGoldCoinRecordPage,{page:1,rows:9}).then((res)=>{
            ...
            success(){
                ...
                this.shows = 2
                this.nowStatus = '1'
                this.get(this.allUrl.findExchangeAppGoldCoinRecordPage,{page:1,rows:9,coinStatus:this.nowStatus}).then((res)=>{
                   this.totalNum = res.data.total       
                    res.data.rows.forEach((i,index)=>{        
                            i.createTime = this.changeTime(i.createTime)
                           i.coinStatus = cgState(i.coinStatus)
            ...
            search(){
                  if(this.value==''){
                     this.$Message.warning('查询内容不能为空！');
                      ...
                      this.get(this.allUrl.findExchangeAppGoldCoinRecordPage,{page:1,rows:9,userName:this.value}).then((res)=>{
                   this.totalNum = res.data.total       
                    res.data.rows.forEach((i,index)=>{        
                           i.createTime = this.changeTime(i.createTime)
                           i.coinStatus = cgState(i.coinStatus)
            ...
            handlePageSize(value){
                     this.pageNum = value;
                 this.get(this.allUrl.findExchangeAppGoldCoinRecordPage,{page:value,rows:9,coinStatus:this.nowStatus,userName:this.value}).then((res)=>{
                   this.totalNum = res.data.total       
                    res.data.rows.forEach((i,index)=>{        
                           i.createTime = this.changeTime(i.createTime)
                           i.coinStatus = cgState(i.coinStatus)
        ...
        beforeMount(){
            ...
            this.get(this.allUrl.countRecycleAppGoldCoinRecord).then((res)=>{         
